...
export default function CustomerConversationsIndex({ chatbot, conversations }) {
  return (
    <CustomerLayout>
      <Head title={`Conversazioni – ${chatbot.name}`} />
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {conversations.data.length === 0 ? (
          <div className="p-12 text-center text-slate-500">
            Nessuna conversazione ancora per questo chatbot.
          </div>
        ) : (
                ...
                <tr key={conv.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="px-4 py-3 text-sm text-slate-600">
                    {conv.first_name || conv.last_name
                      ? [conv.first_name, conv.last_name].filter(Boolean).join(' ')
                      : '–'}
                  </td>
                  <td className="px-4 py-3 text-sm text-slate-600">
                    {conv.email || '–'}
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-right text-sm">
                    <Link
                      href={`/dashboard/chatbots/${chatbot.id}/conversations/${conv.id}`}
